...
def publication_type(source_type: str, doi: str) -> str | None:
    """Return publication type or None to exclude."""

    if source_type == "repository":
        return None

    # Conference proceedings by DOI prefix
    conference_prefixes = (
        "10.2514/6.",   # AIAA
    )

    if doi.startswith(conference_prefixes):
        return "conference"
...
def to_entry(work: dict) -> dict | None:
    ...
    pub_type = publication_type(source_type, norm_doi(doi))
